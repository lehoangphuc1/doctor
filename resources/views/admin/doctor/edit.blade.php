				...
				<form class="forms-sample" action="{{route('doctor.update',[$user->id])}}" method="post" enctype="multipart/form-data" >
					...
					<div class="row">
						<div class="col-lg-6">
							...
							<input type="text" name="name" class="form-control @error('name') is-invalid @enderror" placeholder="Enter doctor name"  value="{{$user->name}}">
						...
						<div class="col-lg-6">
							...
							<input type="email" name="email" class="form-control @error('email') is-invalid @enderror" placeholder="Enter doctor email" value="{{$user->email}}">
							  @error('email')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
					...
					<div class="row">
						<div class="col-lg-6">
							...
							<input type="password" name="password" class="form-control @error('password') is-invalid @enderror" placeholder="Enter doctor password">
							 @error('password')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="department" class="form-control @error('department') is-invalid @enderror">
                                <option value="">Select</option>
                                @foreach(['Cagiologist','Family-Physician','Ophthalmologist','Neurologist','Dentist'] as $depart)
                                <option value="{{$depart}}" @if($user->department==$depart) selected @endif>{{$depart}}</option>
                                @endforeach
                        </select>
                         @error('department')
                                    <span class="invalid-feedback" role="alert">
